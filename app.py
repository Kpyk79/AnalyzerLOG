...
• Остання валідна GPS: {gps_lat:.7f}, {gps_lon:.7f} (t={gps_t}с)
• Тривалість польоту: {flight_s}с, макс. висота: {max_h}м

DEAD RECKONING — браузер проінтегрував курс × швидкість × dt за {dr_dur_s}с після втрати GPS ({dr_dist_own}м пройдено):
→ Розрахована позиція кінця запису: {dr_lat:.7f}, {dr_lon:.7f}
  Це де дрон знаходився в КІНЦІ телеметрії (ще на висоті {last_h}м, не на землі).
  DR має накопичену похибку — враховуй це при оцінці.

СТАТИСТИКА ПІСЛЯ ВТРАТИ GPS ({post_dur}с):
• Середній курс: {post_avg_hdg:.1f}°  (загальний напрямок руху без GPS)
...
- Вітер враховуй тільки для некерованої фази (після кінця телеметрії).
