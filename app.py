...
def build_prediction_prompt(dr, fs, home_lat, home_lon,
    ...
    # Human-readable label for height at last telemetry point
    if incident_h == 0:
        h_label = "0 м — дрон вже на землі в момент обриву телеметрії (аварія при посадці)"
    elif incident_h < 10:
        h_label = f"{incident_h} м — низька висота (аварія на посадці / при зльоті)"
    else:
        h_label = f"{incident_h} м"
    ...
    if manual_home:
        ...
        phase1_block = (
            f"ФАЗА 1 — Весь маршрут від ручної стартової точки ({dr_dur_s}с, {dr_dist_own}м):\n"
            f"  Інтегровано всю телеметрію від t=0 → до кінця запису.\n"
            f"  Середній курс: {post_avg_hdg:.1f}°, середня швидкість: {post_avg_spd:.2f} м/с\n"
            f"  ★ GPS-швидкість з логу вже включає вплив вітру (принцип «бігова доріжка»).\n"
            f"    Якщо вітер > швидкість дрона і проти курсу → рух назад по GPS.\n"
            f"  Остання відома позиція: {incident_lat:.7f}, {incident_lon:.7f} "
            f"(висота {h_label})\n"
            f"  ⚠ Похибка DR зростає з тривалістю інтеграції від ручної точки."
        )
    elif no_gps:
        gps_block = (
        ...
        phase1_block = (
            f"ФАЗА 1 — Весь маршрут (лог без GPS, {dr_dur_s}с, {dr_dist_own}м):\n"
            f"  Інтегровано від стартової точки → до кінця запису.\n"
            f"  Середній курс: {post_avg_hdg:.1f}°, середня швидкість: {post_avg_spd:.2f} м/с\n"
            f"  Остання відома позиція: {incident_lat:.7f}, {incident_lon:.7f} "
            f"(висота {h_label})"
        )
        ...
        phase1_block = (
            f"ФАЗА 1 — Маршрут після GPS-втрати ({post_dur}с, {dr_dist_own}м):\n"
            f"  Інтегровано телеметрію від GPS-фіксу → до кінця запису.\n"
            f"  Середній курс: {post_avg_hdg:.1f}°, середня швидкість: {post_avg_spd:.2f} м/с\n"
            f"  ★ Наземна швидкість GPS-похідна — вітер вже закладено у вектор руху.\n"
            f"    Якщо вітер > швидкість дрона і проти курсу → дрон рухається назад.\n"
            f"  Остання відома позиція: {incident_lat:.7f}, {incident_lon:.7f} "
            f"(висота {h_label})"
        )

    # Phase 2 description depends on remaining height
    if incident_h == 0:
        phase2_note = "Висота = 0 м → дрон приземлився до кінця телеметрії. Вітровий знос = 0."
    elif incident_h < 10:
        phase2_note = (
            f"Мала висота ({incident_h}м) → аварія при посадці. "
            f"Знос вітром мінімальний: {wind_drift_m}м."
        )
    else:
        phase2_note = (
            f"Час падіння: {incident_h}м ÷ {descent_rate:.1f}м/с = {fall_time_s:.1f}с  "
            f"→ знос {wind_drift_m}м у напрямку {wind_to}° ({_deg_to_compass(wind_to)})."
        )

    phase2_block = (
        f"ФАЗА 2 — Залишковий спуск від кінця телеметрії:\n"
        f"  Висота в останній точці запису: {h_label}\n"
        f"  {phase2_note}\n"
...
ТВОЄ ЗАВДАННЯ
════════════════════════════════════════════════════════
1. ВИЗНАЧ СЦЕНАРІЙ аварії за телеметрією:
   а) Втрата зв'язку / РЕБ на висоті → ATTI-дрейф → автопосадка (1.5–2.5 м/с)
   б) Пошкодження / влучання → некерований спуск (4–9 м/с, менший знос вітром)
   в) Критичний АКБ → примусова посадка (1.5–2.0 м/с)
   г) Аварія при посадці → висота в кінці телеметрії ≈ 0–5 м → знос мінімальний
   ОЗНАКИ: pitch/roll > 30° = некерований; V_z < -2.5 = швидкий спуск

2. ПЕРЕВІР Фазу 1 (інтеграція до кінця телеметрії):
   — Остання відома позиція: {incident_lat:.5f}, {incident_lon:.5f} — чи правдоподібна?
...
3. ПЕРЕВІР Фазу 2 (залишковий спуск від висоти {incident_h}м):
   — Якщо висота ≈ 0 → дрон вже приземлився, Фаза 1 дає фінальну точку без корекцій.
   — Якщо висота > 0 → обраний темп {descent_rate:.1f} м/с — чи відповідає сценарію?
   — Час {fall_time_s:.1f}с, знос {wind_drift_m}м у напрямку {wind_to}° — перевір.
