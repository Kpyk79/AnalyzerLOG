...
def build_prompt(summary, filename, mission_type="ВТРАТА"):
    ...
    # Build readable key messages block
    msgs_block = "\n".join(
    ...
    section4_title = "ПРИЧИНИ ВТРАТИ БОРТУ (ДІАГНОСТИКА)" if mission_type == "ВТРАТА" else "ПРИЧИНИ ПОШКОДЖЕННЯ БОРТУ (ДІАГНОСТИКА)"
    crash_word = "аварією та втратою борту" if mission_type == "ВТРАТА" else "аварією з пошкодженням"

    technical_conclusion_instructions = f"""
ТЕХНІЧНИЙ ВИСНОВОК (ЗА ВСТАНОВЛЕНИМ ЗРАЗКОМ) — ФОРМАТ ЖОРСТКО ФІКСОВАНИЙ:

Абзац 1 (РЕБ-джамінг та ATTI):
Починаючи з {s.get('jamming_start_s', '—')} секунди польоту ({s.get('jamming_start_local', '—')}), спостерігалася критична деградація навігаційного сигналу через вплив засобів РЕБ. На борту БпЛА не встановлена модифікована прошивка «1001», тому система відпрацювала за штатними алгоритмами виробника (DJI), автоматично перевівши апарат у режим ATTI (ручне керування без GPS-стабілізації) через втрату супутників.
[ЯКЩО немає jamming — адаптуй цей абзац до фактичних даних]

Абзац 2 (GNSS-спуфінг):
...
[ЯКЩО немає спуфінгу — опиши фактичну аномалію]

Абзац 3 (характер атаки):
...
Абзац 4 (телеметрія завершення + удари):
...
Абзац 6 (дії оператора щодо ATTI):
По даним з лог-файлів дії оператора щодо управління БпЛА в частині переведення його на ручний режим польоту (ATTI) зафіксовані не були. Перехід у режим ATTI відбувся автоматично за алгоритмом DJI при втраті зв'язку з супутниками. Проте, оскільки оператор не зафіксував цей режим примусово на пульті керування, при появі перших фальшивих сигналів спуфінгу система спробувала повернутися до GPS-стабілізації, що і призвело до неконтрольованого ривка та подальшого падіння на висоті {s.get('final_height_m', '—')} м.

Абзац 7 (висновок):
Своєчасна реакція оператора та примусовий перехід у ручний режим польоту до входу в зону дії спуфінгу могли б дозволити польотному контролеру ігнорувати хибні дані. Але в умовах відсутності спеціалізованого ПЗ (як-от «1001»), що блокує прийом GPS на програмному рівні, апарат залишився вразливим до динамічної підміни координат на дистанції {s.get('max_distance_m', '—')} м від точки зльоту.
"""
...
Кваліфікація дій: [ВІРНІ / ЧАСТКОВО ВІРНІ / ТАКТИЧНА ПОМИЛКА — вибери одне]

[Аналіз. Згадай чи оператор переходив у ATTI вручну (якщо atti_times є — значить система перейшла автоматично).]

{technical_conclusion_instructions}
===END_REPORT==="""
